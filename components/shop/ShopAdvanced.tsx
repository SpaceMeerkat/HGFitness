    ...
    return (
        <ImageBackground source={image} resizeMode="cover" style={{ flex: 1, width: "100%", height: "100%" }}>
            <ScrollView style={ShopStyles.shopScrollContainer}>
                <Text style={[DefaultTabStyles.defaultBoldText, {paddingTop: 16}]}>Advanced Programs</Text>
                <View style={ShopStyles.cardBlockContainer}>
                    {advancedPrograms && Object.keys(advancedPrograms).map((programKey, index) => {
                        const programDetails = advancedPrograms[programKey];
                        return (
                            <GymCard
                                key={index}
