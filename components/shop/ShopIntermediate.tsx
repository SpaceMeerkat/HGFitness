    ...
    return (
        <ImageBackground source={image} resizeMode="cover" style={{ flex: 1, width: "100%", height: "100%" }}>
            <ScrollView style={ShopStyles.shopScrollContainer}>
                <Text style={[DefaultTabStyles.defaultBoldText, {paddingTop: 16}]}>Intermediate Programs</Text>
                <View style={ShopStyles.cardBlockContainer}>
                    {intermediatePrograms && Object.keys(intermediatePrograms).map((programKey, index) => {
                        const programDetails = intermediatePrograms[programKey];
                        return (
                            <GymCard
                                key={index}
