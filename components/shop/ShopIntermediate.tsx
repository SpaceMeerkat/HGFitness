...
export function IntermediatePrograms({handleBackButton}: ShopProgramProps) {
    ...
    useEffect(() => {
        const initialFilteredPrograms = filterAndSortPrograms(intermediatePrograms, {
            alphabetical: false,
    ...
    return (
        <ImageBackground source={image} resizeMode="cover" style={{ flex: 1, width: "100%", height: "100%" }}>
            <ScrollView style={ShopStyles.shopScrollContainer}>
                ...
                <View style = {{flex: 0.8}}>
                    <Text style={[DefaultTabStyles.defaultBoldText, {textAlignVertical: 'center'}]}>Intermediate Programs</Text>
                </View>
                <Pressable onPress={() => setFilterModalVisible(true)} style={{flex:0.2, justifyContent: 'center', alignItems: 'flex-end'}}>
                    <Ionicons name="filter" size={24} color="white" />
                </Pressable>
                </View>
                <FilterSortModal visible={filterModalVisible} onClose={() => setFilterModalVisible(false)} programs={intermediatePrograms} setFilteredPrograms={setFilteredPrograms}/>
